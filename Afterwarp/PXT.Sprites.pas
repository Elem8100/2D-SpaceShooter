...
uses
  Windows, System.Types, Classes, SysUtils, Math, Generics.Collections, PXT.Types, PXT.TypesEx,
...
type
  ...
  private
    ...
    FImageName: string;
    FImageLib: TDictionary<string, TTexture>;
    FImageIndex: Integer;
    FMoved: Boolean;
    FTruncMove: Boolean;
    FCollidePos: TPoint;
  ...
  public
    ...
    property ImageLib: TDictionary<string, TTexture> read FImageLib write FImageLib;
...
//    property PatternCount: Integer read GetPatternCount;
  ...
  public
    ...
    procedure SetAnim(AniImageName: string; AniStart, AniCount: Integer; AniSpeed: Single; AniLooped,
      DoMirror, DoAnimate: Boolean; PlayMode: TAnimPlayMode = pmForward); overload; virtual;
    procedure SetAnim(AniImageName: string; AniStart, AniCount: Integer; AniSpeed: Single; AniLooped:
  ...
  public
    ...
    procedure Dead;
    function Select(Point: TPoint; Filter: array of TSpriteClass; Add: Boolean = False): TSprite;
      overload;
    ...
    procedure GroupSelect(const Area: TRect; Filter: array of TSpriteClass; Add: Boolean = False);
      overload;
    ...
    property Canvas: TGameCanvas read FCanvas write FCanvas;
  end;
...
begin
  ...
  FDoCollision := False;
  FMoved := True;
  FBlendingEffect := TBlendingEffect.Normal;
  FVisible := True;
  TruncMove := True;
  FTag := 0;
...
begin
  Result := FImageLib[FImageName].Parameters.Width;
end;

function TSprite.GetImageHeight: Integer;
begin
  Result := FImageLib[FImageName].Parameters.Height;
end;
...
begin
  ...
  case FTruncMove of
    True:
      ...
      FEngine.FCanvas.Draw(FImageLib[FImageName], Round(FX + FWorldX - FEngine.FWorldX), Round(FY +
        FWorldY - FEngine.FWorldY), False, FBlendingEffect);

    False:
      FEngine.FCanvas.Draw(FImageLib[FImageName], FX + FWorldX - FEngine.FWorldX, FY + FWorldY -
        FEngine.FWorldY, False, FBlendingEffect);
...
begin
 ...
 // if CompareText(FImageName, Value) <> 0 then
  ...
  //  FImageName := Value;
   //  if FEngine <> nil then
 //    FImageName := FEngine.FImageLib[FImageName];
  end;
end;
...
procedure TSpriteEx.CircleToAngle(Angle, LookAtX, LookAtY: Integer; RotateSpeed, MoveSpeed: Single;
  DoLookAt: Boolean);
begin
  if DoLookAt then
...
// but can be custom angle
procedure TSpriteEx.CircleToPos(DestX, DestY, LookAtX, LookAtY: Integer; RotateSpeed, MoveSpeed:
  Single; DoLookAt: Boolean);
begin
  if DoLookAt then
    LookAt(LookAtX, LookAtY);
  FDestAngle := Trunc(Angle256(DestX - Trunc(X), DestY - Trunc(Y)));
...
begin
  if not FImageLib.ContainsKey(FImagename) then
    Exit;
  case FDrawMode of
    // 1 color mode
    0:
      case TruncMove of
        True:
          FEngine.FCanvas.DrawColor1(FImageLib[FImageName], Round(FX + FWorldX + Offset.X - FEngine.FWorldX),
            Round(FY + FWorldY + Offset.Y - FEngine.FWorldY), ScaleX, ScaleY, FMirrorX, ARGB(FAlpha,
            FRed, FGreen, FBlue), FBlendingEffect);
        False:
          FEngine.FCanvas.DrawColor1(FImageLib[FImageName], FX + FWorldX + Offset.X - FEngine.FWorldX,
            FY + FWorldY + Offset.Y - FEngine.FWorldY, ScaleX, ScaleY, FMirrorX, ARGB(FAlpha, FRed,
            FGreen, FBlue), FBlendingEffect);
      end;
    ...
    1:
      case TruncMove of
        ...
        True:
          FEngine.FCanvas.DrawRotateC(FImageLib[FImageName], Round(FX + FWorldX + Offset.X - FEngine.FWorldX),
            Round(FY + FWorldY + Offset.Y - FEngine.FWorldY), FAngle, ScaleX, ARGB(FAlpha, FRed,
            FGreen, FBlue), FBlendingEffect);
        False:
          FEngine.FCanvas.DrawRotateC(FImageLib[FImageName], FX + FWorldX + Offset.X - FEngine.FWorldX,
            FY + FWorldY + Offset.Y - FEngine.FWorldY, FAngle, ScaleX, ARGB(FAlpha, FRed, FGreen, FBlue), FBlendingEffect);
      end;
    {
      //4 color mode
...
begin
  Result := PtInRect(Rect(FActiveRect.Left - Trunc(FEngine.WorldX), FActiveRect.Top - Trunc(FEngine.WorldY),
    FActiveRect.Right - Trunc(FEngine.WorldX), FActiveRect.Bottom - Trunc(FEngine.WorldY)), Point(Trunc
    (MouseX), Trunc(MouseY)));
end;
  ...
  case FDrawMode of
    ...
    1:
      case TruncMove of
        ...
        True:
          FEngine.FCanvas.DrawPatternRotate(FImageLib[FImageName], Round(FX + FWorldX + Offset.X -
            FEngine.FWorldX), Round(FY + FWorldY + Offset.Y - FEngine.FWorldY), FAngle, ScaleX,
            PatternIndex, PatternWidth, PatternHeight, FMirrorX, ARGB(FAlpha, FRed, FGreen, FBlue),
            FBlendingEffect);
        False:
          FEngine.FCanvas.DrawPatternRotate(FImageLib[FImageName], FX + FWorldX + Offset.X - FEngine.FWorldX,
            FY + FWorldY + Offset.Y - FEngine.FWorldY, FAngle, ScaleX, PatternIndex, PatternWidth,
            PatternHeight, FMirrorX, ARGB(FAlpha, FRed, FGreen, FBlue), FBlendingEffect);
      end;
    {
      //4 color mode
...
procedure TAnimatedSprite.SetAnim(AniImageName: string; AniStart, AniCount: Integer; AniSpeed:
  Single; AniLooped, DoMirror, DoAnimate: Boolean; PlayMode: TAnimPlayMode = pmForward);
begin
...
procedure TAnimatedSprite.SetAnim(AniImageName: string; AniStart, AniCount: Integer; AniSpeed:
  Single; AniLooped: Boolean; PlayMode: TAnimPlayMode = pmForward);
begin
  ImageName := AniImageName;
  FAnimStart := AniStart;
  FAnimCount := AniCount;
...
begin
  ...
  begin
    if FImageLib.ContainsKey(FImageName) then
    begin
      var ColCount := FImageLib[FImageName].Parameters.Width div FPatternWidth;
      var RowCount := FImageLib[FImageName].Parameters.Height div FPatternHeight;
...
procedure TCustomAnimSprite.SetAnim(AniImageName, AFrameName: string; AniSpeed: Single; AniLooped,
  DoMirror, DoAnimate: Boolean);
begin
  ImageName := AniImageName;
      ...
      begin
        ...
        begin
          ...
          case FTileMode of
            tmHorizontal:
              begin
                FEngine.FCanvas.Draw(FImageLib[ImageName], cx * ChipWidth + OfsX - Offset.X, _y -
                  Offset.Y, FMirrorX, FBlendingEffect);
              end;
            tmVertical:
              begin
                FEngine.FCanvas.Draw(FImageLib[ImageName], _x - Offset.X, cy * ChipHeight + OfsY -
                  Offset.Y, FMirrorX, FBlendingEffect);
              end;
            tmFull:
              begin
                FEngine.FCanvas.Draw(FImageLib[ImageName], cx * ChipWidth + OfsX - Offset.X, cy *
                  ChipHeight + OfsY - Offset.Y, FMirrorX, FBlendingEffect);
              end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // if c >= 0 then
        if DrawMode = 0 then
          FEngine.FCanvas.Draw(FImageLib[FImageName], cx * ChipWidth + OfsX - Offset.X, cy *
            ChipHeight + OfsY - Offset.Y, FMirrorX, FBlendingEffect);

        if DrawMode = 1 then
          FEngine.FCanvas.DrawRotateC(FImageLib[FImageName], cx * ChipWidth + OfsX - Offset.X, cy *
            ChipHeight + OfsY - Offset.Y, FAngle, 1,ARGB(FAlpha,FRed,FGreen,FBlue), FBlendingEffect);
...
begin
  ...
  begin
    if ImageName <> ' ' then
      Result := Bounds(Trunc(-Engine.WorldX - X), Trunc(-Engine.WorldY - Y), Width * FMapWidth,
        Height * FMapHeight)
    else
      Result := Rect(0, 0, 0, 0);
  end;
...
procedure TSpriteEngine.GroupSelect(const Area: TRect; Filter: array of TSpriteClass; Add: Boolean =
  False);
var
...
function TSpriteEngine.Select(Point: TPoint; Filter: array of TSpriteClass; Add: Boolean = False):
  TSprite;
var
  Index, Index2: Integer;
begin
